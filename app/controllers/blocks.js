...
exports.list = function(req, res) {
  ...
  bdb.getBlocksByDate(gte, lte, limit, function(err, blockList) {
      ...
      async.mapSeries(blockList,
        function(b, cb) {
          ...
          allblocks.sort(
            ...
            //for (var i=0;i<6 && i<allblocks.length; i++) {
            // allblocks[i].blockTimeSum = "Test";
          //  }
	var j=0;
	var blockSum=0;
	var blockDiff=0;
	...
	var average =0;
	var minuteDec= 0;
           for (var i=0;i<11 && i<allblocks.length; i++) {
		 lastBlockT[i]=allblocks[i].time;
		
	    	
		blockDiff = lastBlockT[j] - allblocks[i].time;
	if (blockDiff < 60) { 
		seconds = blockDiff;
		minutes = 0;
		result[i] =parseFloat((blockDiff).toFixed(4))}
         else { 	
	   result[i] =parseFloat((blockDiff /60).toFixed(4));
	   minutes = Math.floor(result[i]);
	   seconds = blockDiff - (minutes *60);
	 }
	if (result[i] < 0){
		allblocks[i]["blockTimeSum"] = "Orphaned";
	}		
       else{ 
	if(result[i] ===0){ allblocks[i]["blockTimeSum"] = "Mining...";}
  	   else {allblocks[i]["blockTimeSum"] = minutes + " min" +" "+ seconds +" secs";} 
	} 
	/*result[i]=parseFloat((blockDiff /60).toFixed(3));;
	allblocks[i]["blockTimeSum"] = result[i];
	if (allblocks[i]["blockTimeSum"] %1===0) {
		allblocks[i]["timeType"] = "seconds";
	}
	  else {
		if (allblocks[i]["blockTimeSum"] != "Orphaned"){
		allblocks[i]["timeType"] = "minutes";}

		else{allblocks[i]["timeType"]= " ";}
	} */
		console.log(blockDiff + "seconds of block");
		console.log(blockDiff + "+" + timeSum + "=");
		timeSum += blockDiff;
		
		allblocks[i]["average5"] = timeSum
		if (i>0) {j= j+1;}
		
		
		console.log(timeSum + "sum up to");
		console.log(allblocks[5].time);
	...
	average = parseFloat(((timeSum/5) /60).toFixed(2));
	minutes = Math.floor(average);
	seconds = parseFloat(((timeSum/5) - (minutes *60)).toFixed(0));
	console.log("average = " + average);
	console.log(minutes + "min " + seconds +"secs");
	allblocks[0]["averageTime5"] = minutes + " min " + seconds + " secs";
